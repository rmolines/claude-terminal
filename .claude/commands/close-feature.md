...
Extrair owner e repo de `git remote get-url origin`. Obter a branch atual:

```bash
BRANCH=$(git branch --show-current 2>/dev/null || git -C "$WORKTREE_PATH" branch --show-current)
```

Usar a tool `mcp__plugin_github_github__list_pull_requests` com:

- `owner`: dono do repo
- `repo`: nome do repo
- `state`: `"closed"`
- `head`: `"<owner>:<branch>"` (formato `owner:branch`)

Extrair o número do primeiro PR mergeado (`mergedAt` não vazio) do resultado.
...
Criar o PR usando a tool `mcp__plugin_github_github__create_pull_request` com:
...
- Se existir e o agente estiver **fora** dela:
  ...
  Após remover a worktree local, invocar a skill `commit-commands:clean_gone` para limpar quaisquer branches locais marcadas como `[gone]` (branch remota já foi deletada pelo `merge_pull_request` no `/ship-feature`).
