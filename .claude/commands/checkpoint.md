...
Se o usuário mencionar que uma questão aberta foi resolvida ou refutada: mover o item do bucket "Resolver antes de começar" ou "A implementação vai
responder" para um novo bucket `**Resolvidas neste checkpoint:**` na seção `## Questões abertas` do plan.md.
