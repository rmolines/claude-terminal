...
| `gh pr merge` sem `-R` em sessão com worktrees | `gh pr merge <N>` falha com "Could not resolve to a PullRequest" quando `gh` detecta o repo errado (ex: dentro de worktree ou sessão com múltiplos remotes) | Sempre usar `gh pr merge <N> -R owner/repo` |
