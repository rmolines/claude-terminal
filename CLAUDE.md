...
| Listas numeradas com code blocks entre itens em .md | MD029 (`ol-prefix`) — markdownlint reinicia contagem após code block e rejeita item seguinte que nao começa em 1 | Usar `**Passo A/B/C**` em bold em vez de lista numerada quando ha code blocks entre os itens |
...
| `NSHostingView` + `rootView` durante layout cycle | Atualizar `hosting.rootView` enquanto o NSPanel está visível dispara `setNeedsUpdateConstraints()` → `postWindowNeedsUpdateConstraints`. No macOS 26, se ocorre durante um layout cycle em andamento, lança `NSException` → `EXC_BREAKPOINT`. Sintoma: crash após horas de uso quando painel HITL está aberto e chega um segundo request com conteúdo diferente. Guard de cache (sessionID + description) não é suficiente — ainda crashava ao trocar de sessão. | Usar `@Observable HITLPanelState` como estado compartilhado: `NSHostingView` é criado uma vez com `HITLPanelView(state:)`; mutações de estado são diffadas internamente pelo SwiftUI sem nunca tocar em `rootView =` |
